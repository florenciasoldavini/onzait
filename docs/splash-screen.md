...
- `shared/splash/animated-splash.tsx` is the branded launch presentation shared by web, iOS, and Android.
- `shared/splash/splash-state.ts` owns the sequence, completed-tagline hold, and fade timings.
...
3. Update both vector paths in `shared/splash/animated-splash.tsx` from the same approved vector source.
